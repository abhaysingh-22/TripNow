...
export const useFareCalculation = () => {
  ...
  const calculateFares = useCallback(async (pickup, dropoff, rideTypes) => {
    console.log("calculateFares called with:", { pickup, dropoff, rideTypes });
    ...
    try {
      ...
      const farePromises = rideTypes.map(async (rideType) => {
        try {
          ...
          console.log(`Requesting fare for ${rideType.name} (${vehicleType})`);
          ...
          if (response.data && response.data.fare) {
            return {
              ...
              fare: Math.round(response.data.fare),
              vehicleType: response.data.vehicleType
            };
          }
          return null;
        } catch (error) {
          console.error(`Error calculating fare for ${rideType.name}:`, error);
  ...
  const getFareForRide = useCallback((rideTypeId) => {
