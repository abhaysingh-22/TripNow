...
function CaptainDetails({ isOnline: propIsOnline, setIsOnline: propSetIsOnline }) {
  ...
  const setIsOnline = propSetIsOnline || ((newStatus) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div 
          className={`rounded-full w-3 h-3 ${isOnline ? 'bg-green-500' : 'bg-red-500'}`}
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.7, 1, 0.7],
          }}
      ...
      >
        {statsConfig.map((stat, index) => (
          ...
          >
            ...
            <div className={`${stat.bgColor} w-6 h-6 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center mb-1 sm:mb-2`}>
              <IconComponent 
                type={stat.icon} 
      ...
      <motion.button
        ...
        className={`w-full py-3 rounded-lg font-medium shadow-md transition-colors duration-300 ${
          isOnline ? 'bg-yellow-400 hover:bg-yellow-500 text-white' : 'bg-green-500 hover:bg-green-600 text-white'
        }`}
